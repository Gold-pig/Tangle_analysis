...
def API(request,url=url):
    ...
    try:
        ...
        request = http.request('POST',url=url, body = stringified, headers= headers)
...
file = folder + '{}'.format(name) + ".dmp"

memDB = Set()
# traversal_queue = [solid_milestone]
traversal_queue = ["CYUJPCNJJOMPY9EOBVWPMUUIZLSZEXHFETCOPZXABYIVFNHEPPPTJZWHEJKK9NE9NXLFMOSOLBXE99999"]
counter = 0
bundlehash = []
with open(file,"w+") as f:
    while traversal_queue:
        ...
        if trunk_transaction_hash == all_nines and branch_transaction_hash == all_nines:
            bundlehash = tx[2349:2430]
