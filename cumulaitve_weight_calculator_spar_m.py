...
# A = sparse.lil_matrix((89175,89175))
# for i in G.edges:
#     A[i[0],i[1]] = True
Origin = nx.adjacency_matrix(G)
leer_matrix ={}
B=[]
print("Compute cumulative sum")
for i, n in enumerate(np.flip(list(nx.topological_sort(G)), axis=0)):
    print(i)
    Stuck = Origin[idxs[n], :] + (Origin[idxs[p], :] for p in list(G.successors(n)) + [n])
    A = Stuck.todense()
...
cumsum = np.sum(B, dtype=np.uint64, axis=0)  # for every coloum
...
no_selector = lambda: []
random_selector = lambda: (list(G.nodes)[random.randint(0, N - 1)] for i in
                           range(1000))  # check random subset -> useful for bigger datasets
